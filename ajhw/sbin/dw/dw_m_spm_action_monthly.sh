...
echo "=======================$script_name at a month $before_month  :hive data process======================================"

hive -e "
set mapred.job.queue.name=produce;

insert overwrite table txfs_dw.dw_m_spm_action_monthly
select '$before_month' statis_month,
       sd.app_id app_id,
       sd.uid uid,
       sd.channel channel,
 ...
 where from_unixtime(cast(cast(sd.spm_time as bigint)/1000 as bigint),'yyyy-MM') = '${before_month}'
