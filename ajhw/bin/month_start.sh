...
#执行dw层脚本
sh $basepath/all/all_dw_m.sh $before_day || error_report_and_exit $script_name $? $LINENO

#执行dm层脚本
sh $basepath/all/all_dm_m.sh $before_day || error_report_and_exit $script_name $? $LINENO

#执行st层脚本
sh $basepath/all/all_st_m.sh $before_day || error_report_and_exit $script_name $? $LINENO

#执行sqoop层脚本
sh $basepath/all/all_sqoop_m.sh $before_day || error_report_and_exit $script_name $? $LINENO
